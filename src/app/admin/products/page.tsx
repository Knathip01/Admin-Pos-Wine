  ...
  return (
    <div className="animate-in" style={{ padding: '28px', maxWidth: '1500px' }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl font-bold text-white mb-1">จัดการสินค้า POS</h1>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="btn-wine flex items-center gap-2 px-4 py-2.5"
          >
            <Wine size={16} /> เพิ่มสินค้า Wine
          </button>

          <button
            onClick={() => openEdit(undefined, 'food')}
            className="btn-wine flex items-center gap-2 px-4 py-2.5"
            style={{ background: 'linear-gradient(135deg, #ea580c, #f97316)', boxShadow: '0 4px 15px rgba(249,115,22,0.35)' }}
          >
            <Plus size={16} /> เพิ่มอาหาร 🍽️
          </button>
          ...
          <div className="glass-card w-full" style={{ maxWidth: '720px', maxHeight: '92vh', overflow: 'auto', margin: '16px' }}>
              <div style={{
                ...
                background: isFoodMode
                  ...
                  : 'linear-gradient(135deg, rgba(139,26,44,0.1), rgba(190,24,93,0.04))'
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{
                    width: 40, height: 40, borderRadius: 12, display: 'flex', alignItems: 'center',
                    justifyContent: 'center', fontSize: 20,
                    background: isFoodMode ? 'rgba(249,115,22,0.15)' : 'rgba(139,26,44,0.2)',
                    border: `1px solid ${isFoodMode ? 'rgba(249,115,22,0.3)' : 'rgba(139,26,44,0.3)'}`
                  }}>
                    {isFoodMode ? '🍽️' : '🍷'}
                  </div>
                  <div>
            ...
            <div className="p-6">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                ...
                <div>
                  ...
                  <input className="wine-input text-sm" placeholder={isWineMode ? 'WN-001' : 'FD-001'} value={editingProduct.sku || ''} onChange={e => setEditingProduct({ ...editingProduct, sku: e.target.value })} />
                </div>
                <div>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  {isFoodMode && foodCategories.length === 0 && (
                    <p style={{ fontSize: 11, color: '#f59e0b', marginTop: 4 }}>⚠ ยังไม่มีหมวดหมู่อาหาร — ไปเพิ่มที่ <strong>จัดการหมวดหมู่</strong> ก่อน</p>
                  )}
                </div>
                ...
                {isWineMode && (
                  <>
                    <div style={{ gridColumn: '1 / -1', borderTop: '1px solid var(--border-color)', paddingTop: '14px', marginTop: '4px' }}>
                      <p className="text-xs font-semibold" style={{ color: 'var(--gold-400)' }}>🍷 ข้อมูลเฉพาะไวน์ / เครื่องดื่ม</p>
                    </div>
                    <div>
                ...
                {isFoodMode && (
                  <>
                    <div style={{ gridColumn: '1 / -1', borderTop: '1px solid rgba(249,115,22,0.2)', paddingTop: '14px', marginTop: '4px', background: 'rgba(249,115,22,0.03)', borderRadius: 8, padding: '14px' }}>
                      <p className="text-xs font-semibold" style={{ color: '#f97316' }}>🍽️ ข้อมูลเพิ่มเติมอาหาร</p>
                    </div>
                    <div>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-secondary)' }}>แบรนด์ / ชื่อร้าน (ถ้ามี)</label>
                      <input className="wine-input text-sm" placeholder="เช่น ร้านแม่มาลัย, Swensen's..." value={editingProduct.brand || ''} onChange={e => setEditingProduct({ ...editingProduct, brand: e.target.value })} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-secondary)' }}>แท็กส่วนผสม / คำอธิบายสั้น</label>
                      <input className="wine-input text-sm" placeholder="เช่น ไก่ทอด, vegan, ไม่มีผงชูรส..." value={editingProduct.winery || ''} onChange={e => setEditingProduct({ ...editingProduct, winery: e.target.value })} />
                    </div>
                  </>
                )}
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={handleSave} disabled={saving || uploading || !editingProduct.name}
                  ...
                  style={{
                    opacity: (!editingProduct.name || saving) ? 0.6 : 1,
                    background: isFoodMode ? 'linear-gradient(135deg,#ea580c,#f97316)' : undefined,
                    boxShadow: isFoodMode ? '0 4px 16px rgba(249,115,22,0.35)' : undefined
                  }}>
